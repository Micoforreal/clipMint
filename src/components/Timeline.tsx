...
import { useState, MouseEvent, useRef, useEffect, JSX } from "react";
...
export default function Timeline({
    ...
    const ruler = () => {
        ...
        return <div className={`flex items-center  ${styles.ruler} border-b border-purple-500/20 my-2`}>{rows}</div>;
    }
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            segmentDivs.push(
                ...
                >
                    ...
                    <div
                        className={`${styles.card} bg-[#441956a1] rounded-3xl`}
                        style={{
    ...
    useEffect(() => {
        setTrackDivs(trackList.map((track, ind) => <div className={' bg-purple-500/5 border  rounded-lg mb-2 relative min-h-100% flex items-center border-purple-500/20 '}>{genTrack(track, ind)}</div>));
    ...
    return (
        <div className={styles.container}
            ...
            ref={containerRef}
        >
            <Droppable droppableId="timeline" isDropDisabled={false} isCombineEnabled={false} ignoreContainerClipping={false}>
                {
                    (provided) => (
                        <div className="timeline min-h-[500px] border-y mt-3 border-purple-500/20 min-w-[700px]" {...provided.droppableProps} ref={provided.innerRef} >
                            <div className={styles.tracks} style={{ minWidth: `${projectDuration * scaleFactor}px`, minHeight: `${containerRef.current?.clientHeight}px` }}>
                                ...
                                <div style={{
                                    ...
                                    className="absolute top-1 bottom-0 w-0.5 bg-red-500  z-20"
